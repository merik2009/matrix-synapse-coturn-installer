...
## ⚙️ Работа с правами доступа

### 🔐 Запуск от root vs обычного пользователя

Скрипт теперь поддерживает запуск как от **обычного пользователя** (с sudo), так и от **root**:

#### ✅ От обычного пользователя (рекомендуется):
```bash
./install.sh
# Скрипт автоматически использует sudo при необходимости
```
...
# Запустить установку
./install.sh
# Скрипт выполняет команды напрямую без sudo
```
...
- ✅ Более безопасно
- ✅ Лучший контроль прав
- ✅ Рекомендуется для рабочих станций
